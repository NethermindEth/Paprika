...
namespace Tree.Crypto
{
    public class KeccakHash
    {
        ...
        public static void ComputeHash(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            int i;
            for (;
                 remainingInputLength >= ROUND_SIZE;
                 remainingInputLength -= ROUND_SIZE, input = input.Slice(ROUND_SIZE))
            {
            ...
            if (input.Length >= TEMP_BUFF_SIZE || input.Length > ROUND_SIZE || ROUND_SIZE + 1 >= TEMP_BUFF_SIZE ||
                ROUND_SIZE == 0 || ROUND_SIZE - 1 >= TEMP_BUFF_SIZE || ROUND_SIZE_U64 * 8 > TEMP_BUFF_SIZE)
            {
                throw new ArgumentException("Bad keccak use");
            }
        ...
        public void Update(Span<byte> array, int index, int size)
        {
            ...
            if (size < 0)
            {
                throw new ArgumentException(
                    "Cannot updated Keccak hash because the provided size of data to hash is negative.");
            }
            else if (index + size > array.Length || index < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "Cannot updated Keccak hash because the provided index and size extend outside the bounds of the array.");
            }
            ...
            // last block and padding
            if (input.Length >= TEMP_BUFF_SIZE || input.Length > _roundSize || _roundSize + 1 >= TEMP_BUFF_SIZE ||
                _roundSize == 0 || _roundSize - 1 >= TEMP_BUFF_SIZE || _roundSizeU64 * 8 > TEMP_BUFF_SIZE)
            {
                throw new ArgumentException("Bad keccak use");
            }
